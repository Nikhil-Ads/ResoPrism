...
export function MindMap({ 
  ...
  if (loading) {
    return (
      <Card className={`border-slate-800/50 bg-slate-900/60 backdrop-blur-sm ${className}`}>
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center h-[500px] bg-slate-900/80 rounded-lg">
            <Loader2 className="h-12 w-12 text-teal-500 animate-spin mb-4" />
            <p className="text-slate-400">Analyzing research results...</p>
            <p className="text-sm text-slate-500 mt-1">Finding themes and connections</p>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <Card className={`border-slate-800/50 bg-slate-900/60 backdrop-blur-sm ${className}`}>
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center h-[500px] bg-slate-900/80 rounded-lg">
            ...
            <p className="text-red-400 font-medium">Failed to generate mind map</p>
            <p className="text-sm text-slate-500 mt-1 text-center max-w-md">{error}</p>
            {onRefresh && (
              <Button onClick={onRefresh} variant="outline" className="mt-4 border-slate-700 hover:bg-slate-800">
      ...
      <Card className={`border-slate-800/50 bg-slate-900/60 backdrop-blur-sm ${className}`}>
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center h-[500px] bg-slate-900/80 rounded-lg">
            ...
            <p className="text-sm text-slate-500 mt-1">Search for research to generate a mind map</p>
          </div>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card className={`border-slate-800/50 bg-slate-900/60 backdrop-blur-sm ${className}`} ref={containerRef}>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="flex items-center gap-2 text-lg text-slate-100">
          <Network className="h-5 w-5 text-teal-500" />
          Research Mind Map
